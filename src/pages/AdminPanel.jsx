...
export default function AdminPanel() {
  ...
  const handleTransferPriceChange = (transferType, bracket, val) => {
    const cleanVal = val.trim();
    const updatedPrices = {
      ...(config.custom_tours?._transfer_prices || {}),
      [transferType]: {
        ...(config.custom_tours?._transfer_prices?.[transferType] || {}),
        [bracket]: cleanVal === '' || cleanVal === '—' ? null : (isNaN(Number(cleanVal)) ? cleanVal : Number(cleanVal))
      }
    };
    setConfig(prev => ({
      ...prev,
      custom_tours: {
        ...(prev.custom_tours || {}),
        _transfer_prices: updatedPrices
      }
    }));
  };

  const saveTransferPrices = async () => {
    setSavingSection('transfers');
    setSaveMsg('');
    const updatedCustomTours = {
      ...(config.custom_tours || {}),
      _transfer_prices: config.custom_tours?._transfer_prices || {}
    };
    const { error } = await supabase.from('pricing_config').update({ custom_tours: updatedCustomTours }).eq('id', 1);
    if (error) setSaveMsg('❌ ' + error.message);
    else { setSaveMsg('✅ Tarifas de traslados guardadas'); }
    setSavingSection('');
    setTimeout(() => setSaveMsg(''), 3000);
  };
  ...
  const generateFinalInvoice = async (inv) => {
    ...
    const customEmails = config?.invoice_emails || 'info@freetourcph.com,buchinisantiago@gmail.com';
  ...
  return (
    <div className="min-h-screen text-white pb-20" style={{ background: 'linear-gradient(135deg, #0a0f1e 0%, #111827 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* ── Matriz de Tarifas de Traslados (DKK) ── */}
        <Section icon="🚍" title="Matriz de Tarifas de Traslados (DKK)">
          <p className="text-sm text-gray-400 mb-1">Precios por trayecto/traslado directo sin IVA (25% VAT). Dejar vacío o con un guion (—) si la capacidad no está disponible.</p>
          <div className="overflow-x-auto mt-4">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-white/10 text-gray-400">
                  <th className="text-left py-3 px-2 font-semibold">Tipo de Traslado</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">1-16 pax</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">17-30* pax</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">31-52 pax</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">53-57 pax</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">58-65 pax</th>
                  <th className="px-2 py-3 text-center font-semibold w-24">66-80 pax</th>
                </tr>
              </thead>
              <tbody>
                {Object.entries(config.custom_tours?._transfer_prices || {}).map(([tType, brackets]) => (
                  <tr key={tType} className="border-b border-white/5 hover:bg-white/2 transition">
                    <td className="py-3 px-2 font-semibold text-orange-400 whitespace-nowrap">{tType}</td>
                    {['1-16', '17-30', '31-52', '53-57', '58-65', '66-80'].map(bracket => {
                      const val = brackets?.[bracket] !== null && brackets?.[bracket] !== undefined ? brackets[bracket] : '';
                      return (
                        <td key={bracket} className="px-1 py-1.5">
                          <input
                            type="text"
                            placeholder="—"
                            className="w-full px-2 py-1.5 bg-black/30 border border-white/10 rounded-lg text-center font-mono text-sm text-white outline-none focus:border-orange-500 transition"
                            value={val}
                            onChange={e => handleTransferPriceChange(tType, bracket, e.target.value)}
                          />
                        </td>
                      );
                    })}
          ...
          <button
            onClick={saveTransferPrices}
            disabled={savingSection === 'transfers'}
            className="mt-4 px-8 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 font-bold text-sm transition disabled:opacity-50"
          >
            {savingSection === 'transfers' ? 'Guardando...' : '💾 Guardar Tarifas de Traslados'}
          </button>
        </Section>
